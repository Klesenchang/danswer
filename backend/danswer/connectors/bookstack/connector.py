...
class BookstackConnector(LoadConnector, PollConnector):
    ...
    def _book_to_document(self, book: dict):
        ...
        return Document(
            ...
            metadata={"type": "book", "updated_at": book.get("updated_at")},
        )

    def _chapter_to_document(self, chapter: dict):
        url = self.bookstack_client.build_app_url(
            "/books/" + chapter.get("book_slug") + "/chapter/" + chapter.get("slug")
        )
        ...
        return Document(
            ...
            metadata={"type": "chapter", "updated_at": chapter.get("updated_at")},
        )
    ...
    def _page_to_document(self, page: dict):
        ...
        url = self.bookstack_client.build_app_url(
            "/books/" + page.get("book_slug") + "/page/" + page_data.get("slug")
        )
        page_html = (
            "<h1>"
        ...
        for endpoint, transform in transform_by_endpoint.items():
            ...
            while True:
                doc_batch, num_results = self._get_doc_batch(
                    endpoint, transform, start_ind, start, end
                )
